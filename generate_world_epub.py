...
def make_epub(data: dict, path: Path) -> None:
    ...
    title = str(data.get("title") or f"Зарубежный журнал · {date_label}")
...
def main() -> None:
    data = load_input()
    issue_date = str(data.get("date") or datetime.now(timezone(timedelta(hours=3))).date())
    dated_name = f"world-{issue_date}.epub"
    out = OUT_DIR / dated_name
    make_epub(data, out)
    shutil.copyfile(out, OUT_DIR / "world-latest.epub")
