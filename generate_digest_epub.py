...
def make_epub(data, out):
    ...
    (tmp/'OEBPS/index.xhtml').write_text(f'<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml"><head><title>{x(title)}</title><link rel="stylesheet" href="style.css" type="text/css"/></head><body><h1>{x(title)}</h1><p>Свежие технологические новости в нормальном связном пересказе — без RSS-мусора и обрывков сайтов.</p><ol class="toc">{toc}</ol></body></html>',encoding='utf-8')
